import React from "react";

const AttributionsPage = () => {
  return (
    <div className="m-5 md:mx-10">
      <h1 className="text-center my-5 text-2xl md:text-3xl lg:text-4xl italic font-semibold">
        Attributions
      </h1>
      <ul className="flex flex-col list-disc gap-4">
        <li>
          <a href="https://www.flaticon.com/free-icons/html" title="html icons">
            Html icons created by Freepik - Flaticon
          </a>
        </li>
        <li>
          <a href="https://www.flaticon.com/free-icons/css" title="css icons">
            Css icons created by Pixel perfect - Flaticon
          </a>
        </li>
        <li>
          <a
            href="https://www.flaticon.com/free-icons/javascript"
            title="javascript icons"
          >
            Javascript icons created by Pixel perfect - Flaticon
          </a>
        </li>
        <li>
          <a
            href="https://iconscout.com/icons/mongodb"
            class="text-underline font-size-sm"
            target="_blank"
          >
            Mongodb
          </a>
          by
          <a
            href="https://iconscout.com/contributors/icon-mafia"
            class="text-underline font-size-sm"
            target="_blank"
          >
            Icon Mafia
          </a>
        </li>
        <li>
          <a
            href="https://iconscout.com/icons/express"
            class="text-underline font-size-sm"
            target="_blank"
          >
            Express
          </a>
          by
          <a
            href="https://iconscout.com/contributors/icon-mafia"
            class="text-underline font-size-sm"
            target="_blank"
          >
            Icon Mafia
          </a>
        </li>
        <li>
          <a
            href="https://iconscout.com/icons/react"
            class="text-underline font-size-sm"
            target="_blank"
          >
            React
          </a>
          by
          <a
            href="https://iconscout.com/contributors/icon-mafia"
            class="text-underline font-size-sm"
          >
            Icon Mafia
          </a>
          on
          <a href="https://iconscout.com" class="text-underline font-size-sm">
            IconScout
          </a>
        </li>
        <li>
          <a
            href="https://iconscout.com/icons/node-js"
            class="text-underline font-size-sm"
            target="_blank"
          >
            Node Js
          </a>
          by
          <a
            href="https://iconscout.com/contributors/icon-mafia"
            class="text-underline font-size-sm"
            target="_blank"
          >
            Icon Mafia
          </a>
        </li>
        <li>
          <a
            href="https://iconscout.com/icons/tailwind-css"
            class="text-underline font-size-sm"
            target="_blank"
          >
            Tailwind Css
          </a>
          by
          <a
            href="https://iconscout.com/contributors/Redmatric%20Studio"
            class="text-underline font-size-sm"
          >
            Termicons
          </a>
          on
          <a href="https://iconscout.com" class="text-underline font-size-sm">
            IconScout
          </a>
        </li>
        <li>
          <a href="https://www.flaticon.com/free-icons/cv" title="CV icons">
            CV icons created by Freepik - Flaticon
          </a>
        </li>
        <li>
          <a
            href="https://www.flaticon.com/free-icons/gmail"
            title="gmail icons"
          >
            Gmail icons created by Pixel perfect - Flaticon
          </a>
        </li>
        <li>
          <a
            href="https://www.flaticon.com/free-icons/github"
            title="github icons"
          >
            Github icons created by Freepik - Flaticon
          </a>
        </li>
        <li>
          <a
            href="https://www.flaticon.com/free-icons/linkedin"
            title="linkedin icons"
          >
            Linkedin icons created by riajulislam - Flaticon
          </a>
        </li>
        <li>
          <a href="https://status.render.com/">Render Logo</a>
        </li>
        <li>
          <a href="https://seeklogo.com/free-vector-logos/netlify">
            Netlify Logo
          </a>
        </li>
        <li>
          <a href="https://cdnlogo.com/logo/vercel_34543.html">Vercel Logo</a>
        </li>
        <li>
          <a href="https://www.flaticon.com/free-icons/code" title="code icons">
            Code icons created by juicy_fish - Flaticon
          </a>
        </li>
        <li>
          <a
            href="https://www.flaticon.com/free-icons/machine-learning"
            title="machine learning icons"
          >
            Machine learning icons created by Eucalyp - Flaticon
          </a>
        </li>
        <li>
          <a
            href="https://www.flaticon.com/free-icons/devops"
            title="devops icons"
          >
            Devops icons created by Freepik - Flaticon
          </a>
        </li>
        <li>
          <a
            href="https://www.flaticon.com/free-icons/laptop"
            title="laptop icons"
          >
            Laptop icons created by vectorsmarket15 - Flaticon
          </a>
        </li>
        <li>
          <a
            href="https://www.flaticon.com/free-icons/data-entry"
            title="data entry icons"
          >
            Data entry icons created by Uniconlabs - Flaticon
          </a>
        </li>
        <li>
          <a
            href="https://www.flaticon.com/free-icons/ar-camera"
            title="ar camera icons"
          >
            Ar camera icons created by flatart_icons - Flaticon
          </a>
        </li>
        <li>
          <a href="https://www.vecteezy.com/free-png/isolated">
            Isolated PNGs by Vecteezy
          </a>
        </li>
      </ul>
    </div>
  );
};

export default AttributionsPage;
